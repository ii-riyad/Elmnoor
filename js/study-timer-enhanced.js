(function(){
    ...
    let currentTime=25*60,totalTime=25*60,isRunning=false,isPaused=false,timerInterval=null;
    ...
    function showAlmostDoneMessage(){
        const speechBubble=document.getElementById('speechBubble'),speechText=document.getElementById('speechText');
        ...
        const messages={
            ...
            en:["Just a few minutes left! Keep going! ⏰","You're close to the end! Don't give up! 🏁","Last minutes! Save your work and submit! 💪"]
        };
    ...
    function updateStats(){
        const sessionsEl=document.getElementById('sessionsToday'),minutesEl=document.getElementById('minutesToday');
        if(sessionsEl&&minutesEl){
            let sessions=parseInt(sessionsEl.textContent)||0,minutes=parseInt(minutesEl.textContent)||0;
            sessions++;
    ...
    function init(){
        ...
        document.querySelectorAll('.preset-btn[data-duration]').forEach(btn=>{
            btn.addEventListener('click',()=>{
                if(elements.sessionDuration){
                    ...
                    document.querySelectorAll('.preset-btn[data-duration]').forEach(b=>b.classList.toggle('active',b===btn));
                    if(!isRunning){
        ...
        document.querySelectorAll('.tool-tab').forEach(tab=>{
            tab.addEventListener('click',()=>{
                ...
                document.querySelectorAll('.tool-content').forEach(content=>content.classList.toggle('active',content.id===`${tabName}Content`));
            });
        });
    }
    ...
    window.studyTimerEnhanced={start:startTimer,pause:pauseTimer,stop:stopTimer,updateDisplay:updateDisplay,isRunning:()=>isRunning,isPaused:()=>isPaused,getState:getTimerState};
})();
