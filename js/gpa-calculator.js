...
function addCourseRow() {
  ...
  const gradeOptions = Object.keys(updatedGradePoints)
...
function updateShahabMessageForGPA(action) {
  ...
  if (action === 'open') {
    message = lang === 'ar' ? 'ممتاز! أداة حساب المعدل جاهزة! 🎓' : 'Great! GPA calculator is ready! 🎓'
  } else if (action === 'add') {
    message = lang === 'ar' ? 'ممتاز! أضفت مادة جديدة! 📝' : 'Great! You added a new course! 📝'
  } else if (action === 'delete') {
    message = lang === 'ar' ? 'تم حذف المادة! ✅' : 'Course deleted! ✅'
  }
