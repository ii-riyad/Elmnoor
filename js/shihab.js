(function(){
    'use strict';
    const state={
        isRunning:false,isPaused:false,isBreak:false,currentTime:0,
        totalTime:25*60,breakTime:5*60,interval:null,messageIndex:0,
        lastMessageTime:0,sessionsToday:0,minutesToday:0,goal:'',prayerReminder:false
    };
    const elements={
    ...
    const messages={
        ar:{
            welcome:["مرحباً! أنا شِهاب، مرشدك الدراسي. ابدأ جلسة دراسة وكن معك!","هيا بنا! ابدأ جلسة جديدة وكن معك خطوة بخطوة.","جاهز للدراسة؟ ابدأ الآن!"],
            start:["ممتاز! بدأنا الجلسة. ركز وكن معك! 💪","رائع! ابدأ بتركيز. أنا هنا لدعمك!","حظاً موفقاً! دعنا ننجز هذا معاً!"],
            ...
            break:["حان وقت الاستراحة! قف، تمدد، خذ نفساً عميقاً. ☕","استراحة مستحقة! تحرك قليلاً واشرب ماء.","وقت الاستراحة! استرخ قليلاً ثم نعود."],
        ...
        en:{
            welcome:["Hello! I'm Shihab, your study guide. Start a study session and I'll be with you!","Let's go! Start a new session and I'll be with you step by step.","Ready to study? Start now!"],
            start:["Excellent! We started the session. Focus and I'm with you! 💪","Great! Start focusing. I'm here to support you!","Good luck! Let's get this done together!"],
            middle:["You're doing great! Keep focusing. 💪","Awesome! You're doing excellent work!","Keep going! You're almost halfway there!"],
            almostDone:["Just a few minutes left! Keep going! ⏰","You're close to the end! Don't give up!","Last minutes! Save your work and submit!"],
            break:["Break time! Stand up, stretch, take a deep breath. ☕","Well-deserved break! Move a bit and drink water.","Break time! Relax a bit then we'll continue."],
            prayer:["⏰ Prayer time! Pause gently, pray, then return.","Prayer time! Take a break for prayer.","Reminder: It's prayer time."],
            noStart:["Why haven't you started yet? Come on, start now! 😤","Don't hesitate! Start the session now.","What are you waiting for? Start studying!"],
            completed:["Excellent! You completed the session successfully! 🎉","Awesome! You're a champion! Session completed.","Congratulations! Successful session!"]
        }
    };
    ...
    function showMessage(category,force=false){
        const lang=document.documentElement.getAttribute('lang')||'ar',langMessages=messages[lang]||messages.ar,categoryMessages=langMessages[category]||[];
        ...
        setTimeout(()=>{if(elements.speechBubble)elements.speechBubble.classList.remove('show');},5000);
    ...
    function updateTimer(){
        ...
        const total=state.isBreak?state.breakTime:state.totalTime,percentage=((total-state.currentTime)/total)*100;
    ...
    function tick(){
        ...
        state.currentTime--;
        if(state.currentTime<=0){completeSession();return;}
        updateTimer();
        const total=state.isBreak?state.breakTime:state.totalTime,remaining=state.currentTime,percentage=(remaining/total)*100;
        ...
        if(remaining===60){showMessage('almostDone',true);setCharacterState('happy');}
    }
    function completeSession(){
        ...
        if(state.isBreak){
            ...
            setCharacterState('happy');
            setTimeout(()=>{setCharacterState('idle');startBreak();},3000);
        }
        updateButtons();
    }
    ...
    elements.presetBtns.forEach(btn=>{
        btn.addEventListener('click',()=>{
            if(btn.dataset.duration){
                ...
                elements.presetBtns.forEach(b=>{if(b.dataset.duration)b.classList.toggle('active',b===btn);});
            }
            if(btn.dataset.break){
                if(elements.breakDuration)elements.breakDuration.value=btn.dataset.break;
                elements.presetBtns.forEach(b=>{if(b.dataset.break)b.classList.toggle('active',b===btn);});
            }
            saveSettings();
        });
    });
    ...
    function loadStats(){
        ...
        if(saved){
            try{
                const stats=JSON.parse(saved),today=new Date().toDateString();
                if(stats.date===today){
    ...
    function checkPrayerTime(){
        ...
        const now=new Date(),hours=now.getHours(),minutes=now.getMinutes(),prayerTimes=[
            {name:'Fajr',hour:5,minute:30},{name:'Dhuhr',hour:12,minute:30},
            {name:'Asr',hour:15,minute:30},{name:'Maghrib',hour:18,minute:0},
    ...
    function init(){
        ...
        resetIdleTimer();
        setInterval(()=>{if(!state.isRunning&&Math.random()>0.7)moveCharacter();},10000);
    }
    ...
    const observer=new MutationObserver(()=>{
        if(state.isRunning){
            const lang=document.documentElement.getAttribute('lang')||'ar',category=state.isBreak?'break':'start';
